...
bool FTobiiEyeTracker::Tick(float DeltaTime)
{
	...
	if (ActivePlayerController.IsValid()
		...
		&& CVarEnableGazePointDebug.GetValueOnGameThread())
	{
	...
	TickEvent.Broadcast(DeltaTime, ActivePlayerController.Get());
	...
	if (GazeTrackerStatus == ETobiiGazeTrackerStatus::UserPresent && ActivePlayerController.IsValid())
	{
		...
		else
		{
			...
			if (NumDataSinceLastUpdate > 0)
			{
				...
				LeftGazeData.WorldGazeDirection.Normalize();
				RightGazeData.WorldGazeDirection.Normalize();
		...
		if (LeftGazeData.bIsGazeDataValid && RightGazeData.bIsGazeDataValid)
		{
			...
			CombinedGazeData.WorldGazeDirection.Normalize();
		}
...
void FTobiiEyeTracker::UpdateWorldSpaceData(float DeltaTime)
{
	if (ActivePlayerController.IsValid() && ActivePlayerController->GetWorld() != nullptr)
	{
		...
		if (!ActivePlayerController->GetWorld()->LineTraceSingleByChannel(CombinedWorldGazeHitData, CombinedGazeData.WorldGazeOrigin
		...
		if (!ActivePlayerController->GetWorld()->LineTraceSingleByChannel(LeftWorldGazeHitData, LeftGazeData.WorldGazeOrigin
		...
		if (!ActivePlayerController->GetWorld()->LineTraceSingleByChannel(RightWorldGazeHitData, RightGazeData.WorldGazeOrigin
...
FTobiiEyetrackingTickedSignature& FTobiiEyeTracker::GetTickEvent()
{
	return TickEvent;
}
