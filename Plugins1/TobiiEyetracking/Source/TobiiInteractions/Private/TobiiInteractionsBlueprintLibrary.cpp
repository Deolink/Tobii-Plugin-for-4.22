...
bool UTobiiInteractionsBlueprintLibrary::IsInfiniteScreenEnabled()
{
	...
	if (EyetrackingEnabledCVar != nullptr && InfiniteScreenEnabledCVar != nullptr && ITobiiCore::IsAvailable())
	{
		return ITobiiCore::GetEyeTracker()->GetGazeTrackerStatus() > ETobiiGazeTrackerStatus::Disabled
...
bool UTobiiInteractionsBlueprintLibrary::IsCleanUIEnabled()
{
	...
	if (EyetrackingEnabledCVar != nullptr && CleanUIEnabledCVar != nullptr && ITobiiCore::IsAvailable())
	{
		return ITobiiCore::GetEyeTracker()->GetGazeTrackerStatus() > ETobiiGazeTrackerStatus::Disabled
			&& EyetrackingEnabledCVar->GetInt()
...
bool UTobiiInteractionsBlueprintLibrary::IsAimAtGazeEnabled()
{
	...
	if (EyetrackingEnabledCVar != nullptr && AimAtGazeEnabledCVar != nullptr && ITobiiCore::IsAvailable())
	{
		return ITobiiCore::GetEyeTracker()->GetGazeTrackerStatus() > ETobiiGazeTrackerStatus::Disabled
			&& EyetrackingEnabledCVar->GetInt()
...
bool UTobiiInteractionsBlueprintLibrary::IsFireAtGazeEnabled()
{
	...
	if (EyetrackingEnabledCVar != nullptr && FireAtGazeEnabledCVar != nullptr && ITobiiCore::IsAvailable())
	{
		return ITobiiCore::GetEyeTracker()->GetGazeTrackerStatus() > ETobiiGazeTrackerStatus::Disabled
			&& EyetrackingEnabledCVar->GetInt()
...
bool UTobiiInteractionsBlueprintLibrary::IsThrowAtGazeEnabled()
{
	...
	if (EyetrackingEnabledCVar != nullptr && ThrowAtGazeEnabledCVar != nullptr && ITobiiCore::IsAvailable())
	{
		return ITobiiCore::GetEyeTracker()->GetGazeTrackerStatus() > ETobiiGazeTrackerStatus::Disabled
			&& EyetrackingEnabledCVar->GetInt()
...
FRotator UTobiiInteractionsBlueprintLibrary::GetInfiniteScreenAngles()
{
	FRotator InfiniteScreenAngles = FRotator::ZeroRotator;
 	if (IsInfiniteScreenEnabled())
 	{
...
bool UTobiiInteractionsBlueprintLibrary::TraceBallisticProjectilePath(const UWorld* World, const FTobiiProjectileTraceData& InputData, TArray<FVector>& OutTracedPath, FHitResult& OutHitResult)
{
	if (World == nullptr)
	{
		return false;
	...
	{
		if (World->SweepSingleByChannel(OutHitResult, StartPoint, EndPoint, FQuat::Identity, InputData.TraceChannel, FCollisionShape::MakeSphere(InputData.TraceRadiusCm), CollisionParams))
		{
