...
{
	InvalidInput					//If no throw target is set, or otherwise bad input
	, NoThrowTarget					//If there is no target to throw at

	, NoPath						//If no valid path could be found
	, BlockedByWorldAndOutOfRange	//A path is mathematically possible, but it is blocked by something and out of range.
	, BlockedByWorldInRange			//A path is mathematically possible and is within range, but it is blocked by something.
	, OutOfRange					//If a valid path could be found, but it requires more throw force than allowed
	, DirectHit						//If a valid path exists and the force requirement is within limits
};
